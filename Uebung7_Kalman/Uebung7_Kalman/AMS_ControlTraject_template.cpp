...
#include "AMS_KalmanFilter.hpp" // Header-Datei der KalmanFilter-Klasse einbinden
...
int main(int argc, char **argv) {
    ...
    const double kII=2.0;  // Regelparameter für Positionsfehler (entspricht I-Anteil)
    ...
    // Winkel berechnen und Soll-Geschwindigkeit initialisieren
    ...
    // Kalman-Filter-Objekt erstellen und Roboterzeiger übergeben
    KalmanFilter kf(&robot);

    int step_count = 0;  // Zähler für die Schleifendurchläufe
    ...
    while( infile >> x_s >> y_s ) { // Schleife über alle Punkte der Trajektorie
        ...
        // Aktuelle Geschwindigkeit und Position aktualisieren
        v = robot.get_v();
        w = robot.get_w();
        ...
        kf.PredictCov(theta, delta, phi);  // Kovarianzvorhersage aufrufen

        if (step_count % 5 == 0) {
            kf.PlotEllipse(x, y);  // Bei jedem fünften Schritt Ellipse plotten
        }
